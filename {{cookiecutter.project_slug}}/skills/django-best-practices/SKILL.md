...
Модели определять в `{{cookiecutter.project_slug}}/models.py`. Наследовать от `models.Model` и добавлять:
...
def index(request):
    return render(request, 'Index', {'name': '{{cookiecutter.project_name}}'})
```
